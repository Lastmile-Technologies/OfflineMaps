...
class OfflineMapsExample(mapView: MapView) {
    private val mapView: MapView
    private var mapDownloader: MapDownloader? = null
    private var mapUpdater: MapUpdater? = null
    private var offlineSearchEngine: OfflineSearchEngine? = null
    private var downloadableRegions: List<Region>? = ArrayList()
    private val mapDownloaderTasks: MutableList<MapDownloaderTask> = ArrayList()
    ...
    fun onDownloadListClicked() {
        ...
        mapDownloader!!.getDownloadableRegions(
            LanguageCode.DE_DE,
            DownloadableRegionsCallback { mapLoaderError, list ->
                if (mapLoaderError != null) {
                    val message = "Downloadable regions error: $mapLoaderError"
                    snackbar.setText(message).show()
                    return@DownloadableRegionsCallback
                }
        ...
        val mapDownloaderTask = mapDownloader!!.downloadRegions(regionIDs,
            object : DownloadRegionsStatusListener {
                override fun onDownloadRegionsComplete(
                    ...
                    if (mapLoaderError != null) {
                        val message = "Download regions completion error: $mapLoaderError"
                        snackbar.setText(message).show()
                        return
                    }
    ...
    fun onSearchPlaceClicked() {
        ...
        offlineSearchEngine!!.search(textQuery, searchOptions, SearchCallback { searchError, list ->
            if (searchError != null) {
            ...
            val message = "Test search found " + list!!.size + " results. See log for details."
            snackbar.setText(message).show()
    ...
    private fun checkForMapUpdates() {
        ...
        mapUpdater!!.retrieveCatalogsUpdateInfo(CatalogsUpdateInfoCallback { mapLoaderError, catalogList ->
            if (mapLoaderError != null) {
                Log.e("CatalogUpdateCheck", "Error: " + mapLoaderError.name)
                return@CatalogsUpdateInfoCallback
            }
    ...
    private fun checkInstallationStatus() {
        ...
        if (persistentMapStatus != PersistentMapStatus.OK) {
            ...
            // Let's try to repair.
            mapDownloader!!.repairPersistentMap(RepairPersistentMapCallback { persistentMapRepairError ->
                if (persistentMapRepairError == null) {
                    Log.d("RepairPersistentMap", "Repair operation completed successfully!")
                    return@RepairPersistentMapCallback
                }
    ...
    private fun logCurrentMapVersion() {
        ...
        try {
            ...
            Log.e("MapLoaderError", "Fetching current map version failed: $mapLoaderError")
        }
    }
}